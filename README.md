...
## Summary
The endpoint `POST /de2api/datasetData/previewData` builds a SQL query from a dataset definition sent in the request. A field of type "calculated field" carries an expression. The server places that expression into the SQL with no checks. As a result, any logged-in user can run arbitrary SQL against any configured datasource.

## Root cause

1. **Missing authorization.** `previewData` (and `enumValueDs`, `getDatasetCount`, `getDatasetTotal`) have no permission annotation. In contrast, the sibling endpoints `tableField` and `previewSql` require `#p0.datasourceId+':read'`.

2. **Missing input validation.** The calculated-field expression is base64-decoded and then concatenated raw into the SQL by `Utils.buildCalcField`. The guard `validateSqlInjectionRisk` is applied to the group-field path, but never to the calculated-field path.

## Proof of concept

Log in as any user. Send a `previewData` request with a one-table dataset plus one calculated field. The calculated field's expression is a subquery:

```sql
(SELECT concat(account,0x3a,pwd) FROM per_user LIMIT 1)
```

SQL generated by the server:

```sql
SELECT ..., (SELECT concat(account,0x3a,pwd) FROM per_user LIMIT 1) AS f_ax_2
FROM (SELECT ... FROM per_user) t LIMIT 100
```
...
The PoC script (`PoC.py`, full source in the Appendix) automates the whole flow: it logs in, picks a datasource and table, and lets you run an arbitrary SQL query.
...
## Impact
- Read all data on any connected datasource, and DataEase's own database (users, password hashes, tokens).
- Read and write files when the database user allows it (verified MySQL `INTO OUTFILE` with `FILE` privilege).
- OS command execution, depending on the datasource engine (e.g. PostgreSQL `COPY ... PROGRAM`, or MySQL with a permissive `secure_file_priv`).
- A low-privilege user gains SQL access to datasources they were never granted.
